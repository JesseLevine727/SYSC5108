...
from cartpole_rl.policy import ActorCritic, infer_actor_critic_hidden_dims
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a PPO CartPole agent.")
...
def main() -> None:
    ...
    hidden_dims = infer_actor_critic_hidden_dims(checkpoint["policy_network"])
    policy_network = ActorCritic(CartPoleEnv.observation_size, CartPoleEnv.action_size, hidden_dims=hidden_dims).to(device)
    ...
    for episode in range(1, args.episodes + 1):
        ...
        for _ in range(max_steps):
            with torch.no_grad():
                ...
                logits, _ = policy_network(obs_tensor)
